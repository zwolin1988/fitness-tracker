...
export function DraftRecoveryBanner({ draft, onRestore, onDiscard }: DraftRecoveryBannerProps) {
  return (
    <div className="fixed inset-x-0 bottom-0 sm:bottom-4 flex justify-center p-4 z-50">
      <div className="w-full max-w-lg bg-background-dark/90 dark:bg-background-light/90 backdrop-blur-sm rounded-xl p-4 shadow-2xl border border-white/10 dark:border-black/10">
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <p className="text-white dark:text-black font-medium text-center sm:text-left flex-grow">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <Button
              variant="secondary"
              onClick={onDiscard}
              className="flex-1 sm:flex-initial h-10 px-5 rounded-lg bg-white/10 dark:bg-black/10 text-white dark:text-black hover:bg-white/20 dark:hover:bg-black/20"
            >
              Odrzuć
            </Button>
          </div>
