...
export const UpdateProfileSchema = z.object({
  name: z
    ...
    .trim(),
  weight: z.number().positive("Weight must be greater than 0").max(500, "Weight must not exceed 500 kg"),
  height: z.number().positive("Height must be greater than 0").max(300, "Height must not exceed 300 cm"),
});
